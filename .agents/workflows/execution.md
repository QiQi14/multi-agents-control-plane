...
6. Run `ai verify <task-id> --base <base> --plan` before implementing — at the latest
   before verification — to confirm diff scope and any evidence-gate selection. Gate with
   `ai verify <task-id> --base <base> --run`: this resolves the project's registered
   ...
   for the Rust gate that is `ai cargo <task-id> --base <base> [--label <evidence-label>] -- <argv...>`.
