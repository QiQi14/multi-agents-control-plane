...
(function () {
  ...
  function graphNode(id, label, kind, group, data, amount) {
    return {
      id: id, label: label, kind: kind, group: group || kind,
  ...
  function workspaceModel() {
    var nodes = [graphNode(
      'workspace:project', 'project workspace', 'workspace', 'workspace',
    ...
    P.clusters.forEach(function (cluster) {
      ...
      itemMap[id] = true;
      nodes.push(graphNode(id, cluster.label, 'crate', cluster.id, {
  ...
  function crateModel(crate) {
    var modules = {};
    P.nodes.forEach(function (node) {
      if (node.crate !== crate) return;
      var moduleName = node.module || '(root)';
      var id = 'module:' + crate + '|' + moduleName;
    ...
    var rootId = 'crate:' + crate;
    var nodes = [graphNode(rootId, crate, 'crate', crate, { type: 'crate', crate: crate },
      Object.keys(modules).reduce(function (sum, key) { return sum + modules[key].nodes; }, 0))];
    var itemMap = {};
    Object.keys(modules).sort().forEach(function (id) {
      var item = modules[id];
      itemMap[id] = true;
      nodes.push(graphNode(id, item.module, 'module', item.module, {
        type: 'module', crate: crate, module: item.module, summary: item
      }, item.nodes));
    });
    var edges;
    if (mode() === 'hierarchy') {
      ...
      edges = aggregateEdges(itemMap, function (id) {
        if (nodeCrate[id] !== crate) return '';
        return 'module:' + crate + '|' + (nodeModule[id] || '(root)');
      });
    }
    return { nodes: nodes, edges: edges, title: crate, omitted: 0 };
  }

  function moduleModel(crate, moduleName) {
    var files = P.files.filter(function (file) {
      return file.crate === crate && (file.module || '(root)') === moduleName;
    });
    var rootId = 'module:' + crate + '|' + moduleName;
    var nodes = [graphNode(rootId, moduleName, 'module', moduleName, {
      type: 'module', crate: crate, module: moduleName
    }, files.reduce(function (sum, file) { return sum + file.nodes; }, 0))];
    var itemMap = {};
    files.forEach(function (file) {
      var id = 'file:' + file.path;
      itemMap[id] = true;
      nodes.push(graphNode(id, file.name, 'file', file.path.split('/').slice(0, -1).join('/'), {
        type: 'file', file: file
      }, file.nodes));
    });
    var edges;
    ...
    return { nodes: nodes, edges: edges, title: crate + ' · ' + moduleName, omitted: 0 };
  }
  ...
  function fileModel(path) {
    ...
    var nodes = [graphNode(rootId, file.name, 'file', file.crate, {
      type: 'file', file: file
    }, file.nodes)];
    Object.keys(included).forEach(function (id) {
      ...
      nodes.push(graphNode(id, node.public.split('::').pop(), node.kind,
        node.path === file.path ? node.kind : node.crate, { type: 'symbol', node: node },
  ...
  function readerHtml(node) {
    ...
    if (type === 'workspace') {
      ...
      primary = '<dl class="project-facts"><dt>Files</dt><dd>' + (cluster ? cluster.files : '—') +
        ...
        (cluster ? cluster.pending.toLocaleString() : '—') + '</dd></dl>';
      ...
      var summary = node.data.summary || {};
      primary = '<dl class="project-facts"><dt>Files</dt><dd>' +
    ...
    var agentBody = raw
      ? '<p class="agent-contract">Exact stdout from the repaired binary for this semantic identity.</p>' +
        '<pre class="agent-output"><code>' + esc(raw.stdout) + '</code></pre>'
      : '<p class="agent-contract">Exact task-local context bundle assembled from persisted index rows. ' +
        'It is not a generated summary.</p><pre class="agent-output"><code>' +
        esc(JSON.stringify(bundle, null, 2)) + '</code></pre>';
    ...
    return '<div class="graph-reader-head project-reader-head"><span class="eyebrow">' +
      ...
      '<div class="graph-reader-body project-reader-body">' +
        ...
        '<section class="project-reader-section agent-view" id="agent-view"><div class="section-title">' +
          ...
          '<span class="truth-pill">exact snapshot</span></div>' + agentBody +
